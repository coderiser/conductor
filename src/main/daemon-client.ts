...
export class DaemonClient {
  ...
  private messageHandlers = new Map<string, ((_msg: DaemonMessage) => void)[]>();
  private requestResolvers: ((_msg: DaemonMessage) => void)[] = [];
  private requestId = 1;
  ...
  private setupSocketHandlers(): void {
    ...
    this.socket.on('close', () => {
      ...
      // Auto-reconnect after 1s if not explicit disconnect
   ...
   */
  on(type: string, handler: (_msg: DaemonMessage) => void): void {
  ...
  off(type: string, handler: (_msg: DaemonMessage) => void): void {
  ...
  private scheduleReconnect(): void {
    ...
    this.reconnectTimer = setTimeout(() => {
      console.log('[DaemonClient] Attempting reconnect...');
      this.connect().catch((err) => {
        console.error('[DaemonClient] Reconnect failed:', err.message);
      });
    }, 1000);
  }
