...
export interface SessionMeta { id: string; agent: string; cwd: string; branch?: string; gitBranch?: string; elapsed: number; running: boolean; status?: string; needsAttention?: boolean; exited?: boolean; ptyId?: string; }
export interface LogEntry { time: string; text: string; color: string; }

interface Props {
  onAddTerminal: (a: string, cwd?: string) => void;
  onKillCurrent: () => void;
  onBroadcast: (_data: string) => void;
  ...
  agentStats?: any[];
  availableEditor?: string | null;
}

export function Sidebar({ onAddTerminal, onKillCurrent, onBroadcast, onSelectSession, onToggleCollapse, stats, sessions, logs, notificationCount = 0, onShowDashboard, onShowNotifications, onShowTasks, onShowContext, worktrees = [], conflicts = null, agentStats = [], availableEditor = null }: Props) {
  ...
  const [scrollback, setScrollback] = useState(() => parseInt(localStorage.getItem('terminal-scrollback') || '5000', 10));
  ...
  return (
    <div style={{ width:'100%', height:'100%', background:'var(--canvas-deep)', borderRight:'1px solid var(--hairline)', display:'flex', flexDirection:'column', overflow:'hidden', fontFamily:'var(--font-sans)' }}>
      {/* Header */}
      <div style={{ padding:'12px 10px' }}>
        <div style={{ display:'flex', alignItems:'center', gap:10 }}>
          ...
          {onToggleCollapse && (
            <button onClick={onToggleCollapse} title="Collapse sidebar (Ctrl+B)" style={{
              background:'none', border:'none', color:'var(--caption)', cursor:'pointer',
              fontSize:16, padding:'2px 6px', opacity:0.7,
            }}>◀</button>
          )}
        </div>
      </div>
      ...
      {/* Scrollable body */}
      <div className="sidebar-scroll" style={{ flex:1, overflow:'auto', padding:'4px 0' }}>
        {/* Stats */}
        {section('Stats', 'stats',
        ...
        {section('Sessions', 'sessions',
          sessions.length === 0
            ...
            : <div style={{ display:'flex', flexDirection:'column', gap:2 }}>
                {sessions.map((s) => {
                  // Match agent stats by session ID (ptyId maps to stats.sessionId)
                  const stat = agentStats.find((a: any) => a.sessionId === s.ptyId);
                  const health = stat?.healthScore;
                  const healthColor = health !== undefined
                    ? (health >= 80 ? 'var(--running)' : health >= 60 ? '#d4a72c' : health >= 40 ? '#d97706' : 'var(--failed)')
                    : 'var(--hairline)';
                  return (
                  <div key={s.id} onClick={() => onSelectSession?.(s.id)} style={{
                    ...
                    <div style={{ display:'flex', alignItems:'center', gap:6 }}>
                      ...
                      {health !== undefined && health < 40 && <span title="Low health" style={{ color:'var(--failed)', fontSize:9, fontWeight:700 }}>!</span>}
                      {/* Editor button */}
                      {availableEditor && (
                        <span title={`Open in ${availableEditor}`}
                          onClick={(e) => { e.stopPropagation(); window.electronAPI.openInEditor(s.cwd); }}
                          style={{ cursor:'pointer', fontSize:10, color:'var(--caption)', marginLeft:'auto', padding:'0 2px', opacity:0.7 }}
                          onMouseEnter={(e) => (e.currentTarget as HTMLElement).style.opacity = '1'}
                          onMouseLeave={(e) => (e.currentTarget as HTMLElement).style.opacity = '0.7'}>{"</>"}</span>
                      )}
                      <span style={{ color:'var(--caption)', fontSize:10, marginLeft: availableEditor ? 0 : 'auto', fontVariantNumeric:'tabular-nums' }}>{fmtElapsed(s.elapsed)}</span>
                    </div>
                    ...
                    {/* Health bar + token/cost for agents with stats */}
                    {stat && (
                      <div style={{ display:'flex', alignItems:'center', gap:8, paddingLeft:14, marginTop:3 }}>
                        <span style={{ color:'var(--caption)', fontSize:9, minWidth:20 }}>{health ?? '--'}</span>
                        <span style={{
                          display:'inline-block', width:60, height:4, borderRadius:2, background:'var(--hairline)',
                          verticalAlign:'middle',
                        }}>
                          <span style={{
                            display:'inline-block', width:`${Math.max(0, health ?? 0)}%`, height:'100%',
                            borderRadius:2, background:healthColor, transition:'width 0.5s',
                          }} />
                        </span>
                        <span style={{ color:'var(--caption)', fontSize:9 }}>
                          {stat.tokenCount >= 1000 ? `${(stat.tokenCount/1000).toFixed(1)}k` : stat.tokenCount} t
                        </span>
        ...
        {section('Settings', 'settings',
          <div style={{ fontSize:11, color:'var(--body)', lineHeight:'18px' }}>
            <div style={{ display:'flex', alignItems:'center', gap:6, marginBottom:4 }}>
              <label style={{ flex:1 }}>Scrollback lines:</label>
              <input
                type="number"
                min="1000"
                max="100000"
                ...
                value={scrollback}
                onChange={(e) => {
                  const val = Math.max(1000, Math.min(100000, parseInt(e.target.value || '5000', 10)));
                  setScrollback(val);
                  localStorage.setItem('terminal-scrollback', String(val));
                }}
                style={{
                  width: 70, background:'var(--canvas)', border:'1px solid var(--hairline)', borderRadius:3,
                  color:'var(--ink)', padding:'2px 6px', fontSize:10, fontFamily:'var(--font-mono)',
                  outline:'none',
                }}
              />
            </div>
            <div style={{ fontSize:9, color:'var(--caption)', fontStyle:'italic' }}>
      ...
      <div style={{ padding:'8px 10px', borderTop:'1px solid var(--hairline)', fontSize:10, color:'var(--caption)', lineHeight:'16px', flexShrink:0 }}>
        <span style={{ color:'var(--ink)' }}>Alt+F1-F9</span> pane &nbsp;
        <span style={{ color:'var(--ink)' }}>Ctrl+N</span> new &nbsp;
        <span style={{ color:'var(--ink)' }}>Ctrl+W</span> close &nbsp;
        <span style={{ color:'var(--ink)' }}>Ctrl+B</span> sidebar &nbsp;
