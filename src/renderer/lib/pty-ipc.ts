...
export const pty = {
  spawn: (agent: string, cwd: string, cols: number, rows: number, agentSessionId?: string, isRestore?: boolean, sessionId?: string): Promise<SessionInfo> =>
    getAPI().invoke('pty_spawn', {
      agent, cwd: cwd || '', cols, rows,
      agentSessionId: agentSessionId || '',
      isRestore: isRestore || false,
      sessionId: sessionId || '',
      agentId: agent,
      useWorktree: true, // IPC handler checks agent config for worktree.enabled
    }),
  ...
  onOutput: (id: string, handler: (_data: string) => void): (() => void) =>
    getAPI().onOutput(id, handler),

  onExit: (id: string, handler: (_code: number) => void): (() => void) =>
    getAPI().onExit(id, handler),

  onSessionIdChanged: (id: string, handler: (_agentSessionId: string) => void): (() => void) =>
