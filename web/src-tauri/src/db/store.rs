...
pub struct SavedSession { pub id: String, pub agent: String, pub cwd: String, #[serde(default)] pub agent_session_id: String }
...
impl DbStore {
    pub fn new() -> Result<Self, Box<dyn std::error::Error>> {
        ...
        // Try to add agent_session_id column (ignore error if already exists)
        let _ = conn.execute_batch("ALTER TABLE sessions ADD COLUMN agent_session_id TEXT NOT NULL DEFAULT '';");
    ...
    pub fn save_layout(&self, layout: &SavedLayout) -> Result<(), rusqlite::Error> {
        ...
        for s in &layout.sessions {
            conn.execute("INSERT INTO sessions (id,agent,cwd,agent_session_id) VALUES (?1,?2,?3,?4)", params![s.id, s.agent, s.cwd, s.agent_session_id])?;
        }
    ...
    pub fn load_layout(&self) -> Result<Option<SavedLayout>, rusqlite::Error> {
        ...
        let mut stmt = conn.prepare("SELECT id, agent, cwd, agent_session_id FROM sessions")?;
        let sessions = stmt.query_map([], |r| Ok(SavedSession { id: r.get(0)?, agent: r.get(1)?, cwd: r.get(2)?, agent_session_id: r.get(3).unwrap_or_default() }))?.filter_map(|r| r.ok()).collect();
