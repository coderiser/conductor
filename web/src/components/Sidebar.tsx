...
export function Sidebar({ onAddTerminal, onKillCurrent, onBroadcast, stats, sessions, logs }: Props) {
  ...
  useEffect(() => {
    (async () => {
      try {
        const agents = await invoke<Array<{id:string;name:string;command:string;installed:boolean}>>('detect_agents');
        setDetected(agents);
      } catch {}
    })();
  }, []);

  const installed = (id: string) => detected.find((a) => a.id === id)?.installed ?? false;
    ...
    <div style={{ width:'25%', minWidth:250, maxWidth:340, background:'var(--canvas-deep)', borderRight:'1px solid var(--hairline)', display:'flex', flexDirection:'column', overflow:'hidden', fontFamily:'var(--font-sans)' }}>
      ...
      <div style={{ padding:'12px 10px' }}>
        <div style={{ display:'flex', alignItems:'center', gap:10 }}>
          <img src="/logo.png" style={{ width:40, height:40, objectFit:'contain', border:'none', outline:'none' }} alt="Conductor" />
          <div>
      ...
      {/* Scrollable body — fills full sidebar width */}
            ...
            {cmds.map((c) => {
              ...
              return (
                <button key={c.key} onClick={c.action} disabled={!c.avail}
                  style={{
                    background: confirming ? 'var(--failed)' : isKill ? 'rgba(248,113,113,0.12)' : c.avail ? 'var(--canvas-soft)' : 'transparent',
